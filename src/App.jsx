...
// 🔥 關鍵修正：不再需要 import useGoogleMaps
...
function App() {
  ...
  const handleFinalCalculate = (whereData) => {
    console.log('最終資料收集完成:', { participants, intent: selectedIntent, whereData });
    setCurrentPage('results');
  };
  
  // 🔥 關鍵修正：簡化載入邏輯
  if (loading) { return <div style={styles.loadingContainer}><div style={styles.loadingText}>載入中...</div></div>; }
  ...
  return (
    <div style={styles.container}>
      ...
      <div style={styles.header}>
        ...
        <p style={styles.subtitle}>
          {currentPage === 'why' ? '選擇這次聚會的目的'
            : currentPage === 'who' ? '輸入大家的位置，找出最佳會合點！'
      ...
      {currentPage === 'results' && (
        <ResultsPage onRestart={() => setCurrentPage('why')} />
      )}
    </div>
  );
}
